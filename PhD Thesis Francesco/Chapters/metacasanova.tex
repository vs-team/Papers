...
public class StatementEvaluator : StatementVisitor
{
  ...
  {
    Type conditionType = condition.Eval(new ExpressionEvaluator());
    Type thenType = _then.Eval(new StatementEvaluator());
    Type elseType = _else.Eval(new StatementEvaluator());
...
public class StatementEvaluator : StatementVisitor
{
  ...
  {
    Type conditionType = condition.Eval(new ExpressionEvaluator());
    Type doType = _do.Eval(new StatementEvaluator());
    if (!conditionType.Equals(new Boolean()))
    {
      throw new TypeException("Invalid condition type");
    }
...
In general, for a language abstraction $\alpha$ (like statements) having a syntactical structure $\sigma_{i}$ having a certain amount of syntactical elements of type $\epsilon_{\sigma_{i_1}}, ..., \epsilon_{\sigma_{i_m}}$ (such as the condition or the statement block in a control structure), the general representation of a hard-coded type rule in a language with discriminated unions is obtained by creating a union type $\alpha$ having a case $\sigma_{i}$ with arguments $\epsilon_{\sigma_{i_j}}$ for each syntactical element.

\begin{lstlisting}[mathescape = true]
type $\alpha$ =
|$\sigma_1$ of $\epsilon_{\sigma_{1_1}} * ... * \epsilon_{\sigma_{1_m}}$
...
|$\sigma_1$ of $\epsilon_{\sigma_{n_1}} * ... * \epsilon_{\sigma_{n_m}}$
\end{lstlisting}
