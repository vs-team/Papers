...
\subsection{Motivation for the Experimental Set-up}
\label{sec:ch_mcnv_languages_experimental_motivation}

In this section, Metacasanova is being compared performance-wise to a scripting language commonly used in game development. This choice derives from the fact that the hard-coded version of Casanova compiler generates code that is faster than C\# \cite{abbadi2015high}. Achieving the same order of performance of Casanova hard-coded in its meta-compiled version would be unlikely, thus the goal of Metacasanova (also according to the research questions) is not to be a ``competitor'' of the hard-coded compiler, but to ease the Casanova language development and extension at the same time keeping performance acceptable in the scope of game development. For this reason, the performance of the code of a Casanova program generated by Metacasanova will always be compared with an idiomatic implementation of the same program in a language used in the industry of game development, and not with the hard-coded version of Casanova.
